...
//Für die Hochladen/Datenbank vorbereiten
	...
	$nachname = ucfirst(DB::escape($_POST['nachname']));
	$vorname = ucfirst(DB::escape($_POST['vorname']));

//Hochladen zu S3

	$bucket = 'abipic';
	$keyname = $klasse.' - '.$nachname.' - '.$vorname.' - '.md5_file($_FILES['userfile']['tmp_name']);

	$tags = http_build_query(['vorname' => $vorname, 'nachname' => $nachname, 'klasse' => $klasse]);
...
//In Datenbank schrieben	
	...
	if (DB::query("INSERT INTO `bilder` (`url`, `klasse`, `name`, `vorname`, `ip`) VALUES ('$url', '$klasse', '$nachname', '$vorname', '$client_ip');")) {
...
//Es wird kein Bild hochgeladen
	...
	if (($amt = DB::query("SELECT count(idbilder) 'a' FROM bilder;")) !== false) {
		$amt = $amt[0]['a'];
	} else {
...
<body>
	...
	<nav>
		<div class="nav-wrapper bbs">
			<a href="#" class="brand-logo center">Abiball Bild</a>
		</div>
	</nav>
	<main>
		<div class="container">
			...
			<p class="center">
				...
				Das Abiball Komitee hat vor, während der Übergabe vom Zeugnis, ein Bild von der Einschulung (oder Kindheit) zu zeigen.
			</p>
			<p class="center">
				...
				<div class="card-content white-text">
					...
					<form enctype="multipart/form-data" method="POST">
						...
						<div class="row">
							...
							<div class="input-field col s12 ">
								<input placeholder="Vorname" id="vorname" name="vorname" type="text" class="validate" required="required">
								<label for="vorname">Vorname</label>
							</div>      

							<div class="input-field col s12 ">
								<input placeholder="Nachname" id="nachname" name="nachname" type="text" class="validate" required="required">
